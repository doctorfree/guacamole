...
# Guacamole build home
TOP="${HOME}/src/guacamole"

# Log Location
LOG="${TOP}/Logs/guacamole_${GUACVERSION}_build.log"
...
# Convenience scripts
[ -d "${HOME}/src/guacamole/bin" ] && {
  export PATH="${HOME}/src/guacamole/bin:${PATH}"
}
...
# tomcat9 is the latest version in the Ubuntu repository
...
# Required packages
have_deps=$(type -p inst_deps)
if [ "${have_deps}" ]; then
  inst_deps
else
  apt-get -y install build-essential libcairo2-dev ${JPEGTURBO} ${LIBPNG} \
      libossp-uuid-dev libavcodec-dev libavformat-dev libavutil-dev \
      ...
      libwebsockets-dev freerdp2-x11 libtool-bin ghostscript dpkg-dev wget crudini
fi

apt-get -y install ${MYSQL} ${LIBJAVA} &>> ${LOG}
...
have_tom=$(type -p inst_tomcat)
if [ "${have_tom}" ]; then
  inst_tomcat
else
  apt-get -y install ${TOMCAT} &>> ${LOG}
fi
...
SSRC="${HOME}/src/guacamole/src/guacamole-server-${GUACVERSION}.tar.gz"
[ -f "${SSRC}" ] || {
  wget -q --show-progress -O ${SSRC} ${SERVER}/source/guacamole-server-${GUACVERSION}.tar.gz
  if [ $? -ne 0 ]; then
...
CWAR="${DOWN}/guacamole-${GUACVERSION}.war"
[ -f "${CWAR}" ] || {
  wget -q --show-progress -O ${CWAR} ${SERVER}/binary/guacamole-${GUACVERSION}.war
  if [ $? -ne 0 ]; then
...
JDBC="${DOWN}/guacamole-auth-jdbc-${GUACVERSION}.tar.gz"
[ -f "${JDBC}" ] || {
  wget -q --show-progress -O ${JDBC} ${SERVER}/binary/guacamole-auth-jdbc-${GUACVERSION}.tar.gz
  if [ $? -ne 0 ]; then
...
if [ "${installTOTP}" = true ]; then
  TOTP="${DOWN}/guacamole-auth-totp-${GUACVERSION}.tar.gz"
  [ -f "${TOTP}" ] || {
    wget -q --show-progress -O ${TOOTP} ${SERVER}/binary/guacamole-auth-totp-${GUACVERSION}.tar.gz
    if [ $? -ne 0 ]; then
...
if [ "${installDuo}" = true ]; then
  ADUO="${DOWN}/guacamole-auth-duo-${GUACVERSION}.tar.gz"
  [ -f "${ADUO}" ] || {
    wget -q --show-progress -O ${ADUO} ${SERVER}/binary/guacamole-auth-duo-${GUACVERSION}.tar.gz
    if [ $? -ne 0 ]; then
...
if [[ -z $LIBJAVA ]]; then
    ...
    # mysql-connector-j_8.1.0-1ubuntu22.04_all.deb
    MDEB="${DOWN}/mysql-connector-java-${MCJVER}.deb"
    [ -f ${MDEB} ] || {
      wget -q --show-progress -O ${MDEB} \
        https://dev.mysql.com/get/Downloads/Connector-J/mysql-connector-j_${MCJVER}-1ubuntu22.04_all.deb
      if [ $? -ne 0 ]; then
      ...
      echo -e "${GREEN}Downloaded mysql-connector-java-${MCJVER}.deb${NC}"
    }
    apt install ${MDEB} -y
else
...
cd ..
mv -f ${DOWN}/guacamole-${GUACVERSION}.war /etc/guacamole/guacamole.war
mv -f \
  ${DOWN}/guacamole-auth-jdbc-${GUACVERSION}/mysql/guacamole-auth-jdbc-mysql-${GUACVERSION}.jar \
